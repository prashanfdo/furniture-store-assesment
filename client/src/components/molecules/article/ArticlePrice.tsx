...
const ArticlePrice: React.FC<ArticlePriceProps> = ({ currency, price, salePrice, discount }: ArticlePriceProps) => {
  ...
  return (
    <div className="text-gray-600">
      ...
      {discountPercent > 0 ? <span className="line-through text-sm ml-4">{regularPrice}</span> : null}
      {discountPercent > 0 ? (
        <span className="bg-red-500 text-sm ml-2 rounded-lg py-1 px-1 text-white">-{discountPercent}%</span>
      ) : null}
    </div>
  );
};
